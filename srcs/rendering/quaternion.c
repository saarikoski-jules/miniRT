...
/*   Updated: 2020/06/22 12:17:47 by jsaariko      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static t_qua	*get_q_values(t_vec *base_u, t_vec *orien_u, t_vec *axis_u)
{
	double	dot;
	double	angle;
	double	half_sin;
...
static t_qua	*parallel_vecs(t_vec *base_u, t_vec *orien_u)
{
	...
	if (base_u->x == orien_u->x && base_u->y == orien_u->y
		&& base_u->z == orien_u->z)
		q->w = 1.0;
	else
		q->w = 0.0;
...
t_qua			*determine_quaternion(t_vec *orien, t_vec *base)
{
	...
	else
	{
		...
		q = get_q_values(base_u, orien_u, axis_u);
		free(axis_u);
	}
	free(orien_u);
	free(base_u);
